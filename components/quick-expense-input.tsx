...
import { parseQuickExpense } from '@/lib/categorize';
...
export function QuickExpenseInput({ onSubmit, monthDate }: Props) {
  ...
  const parsed = parseQuickExpense(raw);
    ...
    <View style={styles.wrap}>
      ...
      {parsed ? (
        <ThemedText style={[styles.preview, { color: palette.textMuted }]}>
          {money(parsed.amount)} · {parsed.description} ·{' '}
          {t(`category.${parsed.category}` as never)}
          {day ? ` · ${t('format.day', { day })}` : ''}
        </ThemedText>
      ) : raw.trim() ? (
        <ThemedText style={[styles.preview, { color: palette.textMuted }]}>
          {t('quick.expense.incomplete')}
        </ThemedText>
      ) : null}
